...
def format_description_mac_mini(description):
    if "M1 " not in description and "M2 " not in description and "M1," not in description and "M2," not in description:
        return None
    """Find chip"""
    pattern = r'(M[12])\s*(?:Chip\b\s*)?(Pro|Max)?'
    matches = re.search(pattern, description, re.IGNORECASE)
    ...
    gpu_match = gpu_regex.search(description)
    if gpu_match:
        gpu = gpu_match.group(1)
        
    else:
        pattern = r'(M[12]).*?(\d+C GPU|\d+core)'

        match = re.search(pattern, description)
        if match:
            gpu = re.search(r'\d+', match.group(2)).group()
    ...
    """Ethernet port"""
    pattern = r'10GbE|10GB Ethernet'
    match_ethernet = re.search(pattern, description, re.IGNORECASE)
    if match_ethernet:
        ethernet = match_ethernet.group()
        ethernet = re.sub(r'\bChip\b', '', ethernet)
        port =  ethernet.strip().title()
        description = description.replace('10GB Ethernet', '')
        description = description.replace('10GbE', '')
    else:
        port = None
    ...
    if port:
        return f'Mini {chip} {gpu}-GPU {memory}/{storage} 10GbE'
    else:
        return f'Mini {chip} {gpu}-GPU {memory}/{storage}'
    


da = '''APPLE Mac mini M2 PRO Z1700003M
...
da2 = '''APPLE Mac mini M2 Z16L0001P

Detailed specification
Apple (ARM) / M2 / Avalanche / APL1109 / LPDDR5 / 8GB / SSD / 1TB / M2 10 core / Built-in speaker / 1Gbps wired / 802.11ax (Wi-Fi 6E) wireless / Bluetooth / HDMI / USB 3.0 (5Gbps) / Thunderbolt 4 / Mac OS Ventura / Mini PC / 1.18kg / Usage: Office/Lecture / Order-made products'''

da3 = '''APPLE Mac mini M2 PRO MNH73KH/A

Detailed specification
Apple (ARM) / M2 PRO / Avalanche / LPDDR5 / 16GB / SSD / 512GB / M2 16 core / Built-in speaker / 1Gbps wired / 802.11ax (Wi-Fi 6E) wireless / Bluetooth / HDMI / USB 3.0 (5Gbps) / Thunderbolt 4 / Mac OS Ventura / Mini PC / 1.28kg / Usage: Office/Lecture'''


ai = 'M2 (8-core CPU, 10-core GPU), 8GB, 256GB'
ai2 = 'M2 Pro (10-core CPU, 16-core GPU), 16GB, 512GB'
ai3 = 'M1, 8GB, 512GB'

tf = 'Apple Mac Mini MMFK3 (2023, M2, 8C CPU, 10C GPU, 8GB, 512GB SSD) Silver (серебристый)'


ref = '''Refurbished Mac mini Apple M2 Chip with 8‑Core CPU and 10‑Core GPU Overview Originally released January 2023 8GB unified memory 256GB SSD1 Two Thunderbolt 4 ports Gigabit Ethernet port'''
ref2 = 'Refurbished Mac mini Apple M2 Chip with 8‑Core CPU and 10‑Core GPU Overview Originally released January 2023 8GB unified memory 512GB SSD1 Two Thunderbolt 4 ports Gigabit Ethernet port'
ref3 = 'Refurbished Mac mini 3.0GHz 6-core Intel Core i5 - Space Gray Overview Originally released October 2018 8GB of 2666MHz DDR4 SODIMM memory 512GB PCIebased SSD1 Four Thunderbolt 3 ports up to 40 Gbps Intel UHD Graphics 630 Gigabit Ethernet port '
ref4 = 'Refurbished Mac mini Apple M2 Pro Chip with 10‑Core CPU and 16‑Core GPU Overview Originally released January 2023 16GB unified memory 512GB SSD1 Four Thunderbolt 4 ports Gigabit Ethernet port '

ref5 = 'Refurbished Mac mini Apple M1 Chip with 8‑Core CPU and 8‑Core GPU Overview Originally released November 2020 16GB unified memory 2TB SSD1 Two Thunderbolt USB 4 ports Gigabit Ethernet port'
ref6 = 'Refurbished Mac mini Apple M1 Chip with 8‑Core CPU and 8‑Core GPU, 10GB Ethernet Overview Originally released November 2020 16GB unified memory 2TB SSD1 Two Thunderbolt USB 4 ports 10 Gigabit Ethernet port'
ref7 = 'Refurbished Mac mini Apple M2 Pro Chip with 12‑Core CPU and 19‑Core GPU, 10GB Ethernet Overview Originally released January 2023 32GB unified memory 2TB SSD1 Four Thunderbolt 4 ports 10 Gigabit Ethernet port'

# print(format_description_mac_mini(ref6))
...
# print(translator.translate(b).text)