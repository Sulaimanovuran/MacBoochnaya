...
currency = round(requests.get('https://www.cbr-xml-daily.ru/daily_json.js').json())



translator = Translator()

# url = 'https://prod.danawa.com/list/?cate=11346301&15main_11_03&_gl=1*1ufmntg*_ga*NzMwODQ5MTkxLjE2ODI4NTQ4NjI.*_ga_L8D5P2KD8Z*MTY4MzQwMDUwNi4xNy4xLjE2ODM0MDA1NzUuNTguMC4w'
...
all_products = soup.find_all('li', class_='prod_item')
    ...
    if description:
        ...
        for price_item, memory_item in zip(prices, memories):
            ...
            if full_desc in macbooks_pro:
                if price < macbooks_pro[full_desc][0]:
                    macbooks_pro[full_desc] = [price, link]
            else:
                macbooks_pro[full_desc] = [price, link]
    else:
        continue

for k,v in macbooks_pro.items():
    print(k,v)
